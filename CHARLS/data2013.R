...
#读取数据
#
biomarkers<-import('~/data/CHARLS/CHARLS2013/Biomarker.dta')
demograph<-import('~/data/CHARLS/CHARLS2013/Demographic_Background.dta')
health<-import('~/data/CHARLS/CHARLS2013/Health_Status_and_Functioning.dta')
health_care<-import('~/data/CHARLS/CHARLS2013/Health_Care_and_Insurance.dta')
...
weight<-import('~/data/CHARLS/CHARLS2013/Weights.dta')
##
demograph2<-demograph%>%transmute(
  ...
  education_2013=factor(case_when(
    ...
    bd001_w2_3>=7 ~ 4),levels=c(1,2,3,4),
    labels=c('No formal education','Primary school','Middle or high school','College or above')),
...
#export(data,'~/CHarls2013.xlsx')
#biomarkers,demograph,health,blood,health_care
###个人数
#summary(data[,c('ID','householdID')])
#nrow(data[which(!duplicated(data$ID)),])
#家庭数量
#nrow(data[which(!duplicated(data$householdID)),])
##验证
#summary(data[,1:12])
rm(biomarkers,biomarkers2,demograph,demograph2,health2,health,health_care,health_care2,household,)
