...
baseline_compare<-biomarker%>%transmute(
  ...
  CA=factor(ifelse(CA==1,1,0),levels = c(0,1),labels=c('Negative','Positive')),CA_type=CA_type,
              ...
              CA_type_female=ifelse(baseline_compare$CA_type==' ',0,ifelse(baseline_compare$CA_type=='宫颈癌' | baseline_compare$CA_type=='乳腺癌' | baseline_compare$CA_type=='输卵管癌' | baseline_compare$CA_type=='子宫内膜癌',1,2))
)
