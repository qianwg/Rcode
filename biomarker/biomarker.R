...
library(moments)##D’Agostino-Pearson test 
...
biomarker.smoking<-biomarker%>% filter(!is.na(smoking))%>%select(smoking,CEA,sex,CEA.group,age_group2)%>%transmute(
                                                smoking=factor(smoking,levels=c(1,2,3),labels=c('Never','Current','Ago')),
                                                CEA=log(CEA),sex=factor(sex,labels=c('man','woman')),
                                                CEA.group=factor(CEA.group,labels=c('negative','positive')),
                                                age.group=factor(age_group2))
