...
shinyServer(function(input, output) {
  ...
  #反应式2：高危人群的得分计算
  risk<-reactive({
    risk_function(dataset())
  })
  #反应式3：高危人群得分2
  name1<-c('id','name','lung_score','breast_score','liver_score','gastric_score')
  name2<-c('id','name','sex','age','persoid','cellphone','telephone','lung_score','breast_score','liver_score','gastric_score')
  risk_sum<-reactive({
    data_risk<-risk_function(dataset())
  ...
  risk_sum3<-reactive({
    ...
    risk_sum[,c('id','name','sex','age','persoid','cellphone','telephone','lung_score','breast_score','liver_score','gastric_score','lung','breast','liver','gastric','total','item')]
  })
  risk_sum2<-reactive({
    risk_sum2<-melt(risk_sum(),id.vars=c('id','name'),variable.name = 'cancer',value.name = 'score')
  })
  ...
  output$table3<-renderDT({
    datatable(dataset(),class="cell-border stripe",caption = '表1：上传原始数据',width = 12,options=list(pageLength=10,autoWidth=TRUE))
  })
  ...
  output$table<-renderDT({
    ...
    if(is.null(inFile))
      return(NULL)
    datatable(risk_sum3(),options=list(),colnames=c('编号','姓名','性别','年龄','身份证号','手机号','座机',
