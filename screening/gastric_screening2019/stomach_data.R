...
pepsinogen<-pepsinogen%>%transmute(
  ...
  stress_risk=factor(ifelse(stress==2,1,0),levels=c(0,1),labels = c('否','是')),
)%>%transmute(ID=ID,PG1,PG2,PGR, PG1_range=PG1_range,PGR_range,PG2_range,#name=name,
              n=as.character(1:length(PG1)),AFP,CA125,CA153,CA199,CEA,HBsAg_pos,PG_pos3,PG_pos4,
              ...
              #人口学特征
              婚姻=marriage_risk,就业状况=employm_risk,
...
pepsinogen2<-pepsinogen%>%filter(残胃!='是',萎缩性胃炎!='是',胃粘膜不典型增生!='是',胃肠上皮化生!="是",幽门螺杆菌感染史!='是',消化性溃疡!="是")
