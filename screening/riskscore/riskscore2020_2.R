risk_function<-function(data){
  ...
  smoking_risk[data$smoking==2 & data$baonian<20]<-2
  smoking_risk[data$smoking==2 & data$baonian>=20]<-3
  smoking_risk[data$smoking==3 & data$baonian<20 ]<-1
  smoking_risk[data$smoking==3 & data$baonian>=20 ]<-2
  #(3)被动吸烟
  #1、从未暴露过（0分）
  #2、暴露过，但暴露时间<10年（1分）
  #3、暴露过，但暴露时间≥10年（2分）
  ...
  #共同因素得分之和
  common_risk<-age_risk+passivesmk_risk+bmi_risk+food_risk+body_risk+stress_risk+air_risk#+smoking_risk
  #----------------------------------------------------------肺癌----------------------------------------------------------
  #肺癌
  lung_fath<-ifelse(!is.na(catpfath),ifelse(catpfath==34,3,0),0)
  lung_moth<-ifelse(!is.na(catpmoth),ifelse(catpmoth==34,3,0),0)
  lung_brot<-ifelse(!is.na(catpbrot),ifelse(catpmoth==34,3,0),0)
  lung_sist<-ifelse(!is.na(catpsist),ifelse(catpmoth==34,3,0),0)
  lung_chil<-ifelse(!is.na(catpchil),ifelse(catpchil==34,3,0),0)
  ...
  lung_family1<-data.frame(lung_fath,lung_moth,lung_brot1,lung_brot2,lung_sist1,lung_sist2,lung_chil1,lung_chil2,lung_brot,lung_sist,lung_chil)
  ...
  lung_disea<-disea_2+disea_3+disea_4+disea_5+disea_6
  ...
  lung_score<-common_risk+lung_family+lung_disea_occuexpo+smoking_risk
  #-----------------------------------------------------2.乳腺癌----------------------------------------------------------------
  ...
  breast_sist<-ifelse(!is.na(catpsist),ifelse(catpsist==47,3,0),0)
  breast_chil<-ifelse(!is.na(catpchil),ifelse(catpchil==47,3,0),0)
  ...
  breast_family1<-data.frame(breast_fath,breast_moth,breast_brot1,breast_brot2,
                             breast_sist1,breast_sist2,breast_chil1,breast_chil2,breast_sist,breast_chil)
  ...
  breast_risk_score<-ifelse(breast_risk>5,5,breast_risk)
  #4.乳腺癌总分
  ...
  liver_brot<-ifelse(!is.na(catpbrot),ifelse(catpbrot==24,3,0),0)
  liver_sist<-ifelse(!is.na(catpsist),ifelse(catpsist==24,3,0),0)
  liver_chil<-ifelse(!is.na(catpchil),ifelse(catpchil==24,3,0),0)
  ...
  liver_family1<-data.frame(liver_fath,liver_moth,liver_brot1,liver_brot2,
                            liver_sist1,liver_sist2,liver_chil1,liver_chil2,liver_brot,liver_sist,liver_chil)
  ...
  gastric_brot<-ifelse(!is.na(catpbrot),ifelse(catpbrot==16,3,0),0)
  gastric_sist<-ifelse(!is.na(catpsist),ifelse(catpsist==16,3,0),0)
  gastric_chil<-ifelse(!is.na(catpchil),ifelse(catpchil==16,3,0),0)
  ...
  gastric_family1<-data.frame(gastric_fath,gastric_moth,gastric_brot1,gastric_brot2,
                              gastric_sist1,gastric_sist2,gastric_chil1,gastric_chil2,gastric_brot,gastric_sist,gastric_chil)
  ...
  #胃癌总分
  gastric_score<-gastric_family+gastric_disea+common_risk+smoking_risk
  ###合并
...
risk_function2<-function(data){
  ...
  smoking_risk[data$smoking==3 & is.na(data$baonian)]<-0
  smoking_risk[data$smoking==2 & data$baonian<20]<-2
  smoking_risk[data$smoking==2 & data$baonian>=20]<-3.5
  smoking_risk[data$smoking==3 & data$baonian<20 & data$quitsmkyrs>15 & data$quitsmkyrs<99]<-1
  smoking_risk[data$smoking==3 & data$baonian<20 & data$quitsmkyrs<=15 ]<-1.5
  smoking_risk[data$smoking==3 & data$baonian>=20 & data$quitsmkyrs>15 & data$quitsmkyrs<99]<-2.5
  smoking_risk[data$smoking==3 & data$baonian>=20 & data$quitsmkyrs<=15]<-3
  #(3)被动吸烟
  #1、从未暴露过（0分）
  #2、暴露过，但暴露时间<10年（1分）
  #3、暴露过，但暴露时间≥10年（2分）
  ...
  #每周至少3次新鲜水产品、薯类、杂粮、豆类、坚果、大蒜、菌类（-1分/个）
  #每周至少1次油炸、烧烤、熏制、腌制、酱制、晒制食品（1分/个）
  food_risk1<-ifelse(is.na(alcohol),0,ifelse(alcohol==2,1,0))#饮酒
  food_risk2<-ifelse(is.na(tea),0,ifelse(tea==2,-1,0))#茶
  food_risk3<-ifelse(is.na(milk),0,ifelse(milk==2,-1,0))#鲜奶
  food_risk4<-ifelse(is.na(yogurt),0,ifelse(yogurt==2,-1,0))#酸奶
  food_risk5<-ifelse(is.na(coffee),0,ifelse(coffee==2,-1,0))#咖啡
  food_risk6<-ifelse(is.na(sodas),0,ifelse(sodas==2,1,0))#碳酸饮料
  food_risk7<-ifelse(is.na(juice),0,ifelse(juice==2,1,0))#果汁饮料
  food_risk8<-ifelse(is.na(teadr),0,ifelse(teadr==2,1,0))#茶味饮料
  food_risk9<-ifelse(is.na(veget),0,ifelse(veget==2,-1,0))#蔬菜
  food_risk10<-ifelse(is.na(fruit),0,ifelse(fruit==2,-1,0))#水果
  food_risk11<-ifelse(is.na(grain),0,ifelse(grain==2,-1,0))#谷类
  food_risk12<-ifelse(is.na(egg),0,ifelse(egg==2,-1,0))#鸡蛋
  food_risk13<-ifelse(is.na(seafd),0,ifelse(seafd==2,-1,0))#水产品
  food_risk14<-ifelse(is.na(potato),0,ifelse(potato==2,-1,0))#薯类
  food_risk15<-ifelse(is.na(cereal),0,ifelse(cereal==2,-1,0))#杂粮
  food_risk16<-ifelse(is.na(beans),0,ifelse(beans==2,-1,0))#豆类
  food_risk17<-ifelse(is.na(nuts),0,ifelse(nuts==2,-1,0))#坚果
  food_risk18<-ifelse(is.na(garlic),0,ifelse(garlic==2,-1,0))#大蒜
  food_risk19<-ifelse(is.na(fungus),0,ifelse(fungus==2,-1,0))#菌类
  food_risk20<-ifelse(is.na(fried),0,ifelse(fried==2,1,0))#烧烤
  food_risk21<-ifelse(is.na(barbecued),0,ifelse(barbecued==2,1,0))
  food_risk22<-ifelse(is.na(smked),0,ifelse(smked==2,1,0))
  food_risk23<-ifelse(is.na(salted),0,ifelse(salted==2,1,0))
  food_risk24<-ifelse(is.na(sauced),0,ifelse(sauced==2,1,0))
  ...
  total<-cbind(data,age_risk,smoking_risk,passivesmk_risk,bmi_risk,food_risk,body_risk,stress_risk,common_risk)
  return(total)
