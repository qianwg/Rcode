...
screening3%>%group_by(出生年份分组)%>%summarise(mean=mean(初次生育年龄,na.rm=T),sd=sd(初次生育年龄,na.rm=T))
#哺乳
...
#初潮年龄
#初潮年龄与出生年份:14.9-13.4
a<-screening3%>%filter(!is.na(初潮年龄))%>%group_by(出生年份)%>%summarise(n=n())
print(a,n=35)
...
#曲线不拟合
screening3%>%filter(初潮年龄>7,出生年份<=1975)%>%group_by(出生年份)%>%summarise(mean=mean(初潮年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean))+geom_line(colour='blue')+
  scale_x_continuous(breaks = seq(1945,1978,5))+mytheme+labs(y='初潮年龄(均值)',title='初潮年龄\n(n=79927)')
...
#不拟合
screening3%>%filter(初潮年龄>7,!is.na(教育),出生年份<=1975)%>%group_by(教育,出生年份)%>%summarise(mean=mean(初潮年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean,color=教育,linetype=教育))+geom_line()+
  scale_x_continuous(breaks = seq(1945,1975,5))+mytheme+labs(y='初潮年龄(均值)',title='教育水平分层\n(n=79918)' )
...
#不拟合
screening3%>%filter(初潮年龄>7,出生年份<=1975)%>%group_by(出生年份,地区)%>%summarise(mean=mean(初潮年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean,color=地区,linetype=地区))+geom_line()+
  scale_x_continuous(breaks = seq(1945,1975,5))+mytheme+labs(y='初潮年龄(均值)',title='地区分层\n(n=79927)')
...
screening3%>%filter(初潮年龄>7,!is.na(教育),出生年份<=1975)%>%group_by(地区,教育,出生年份)%>%summarise(mean=mean(初潮年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean,color=教育,linetype=教育))+
  geom_line()+facet_wrap(.~地区,nrow=1,labeller = labeller(地区=region))+
  scale_x_continuous(breaks = seq(1945,1975,5))+mytheme+labs(y='初潮年龄(均值)')
...
a2<-screening3%>%group_by(生育年份)%>%summarise(n=n())
a3<-screening3%>%filter(生育年份<2011,生育年份>=1965)%>%group_by(生育年份)%>%summarise(mean=mean(生育次数,na.rm=TRUE))
print(a2,n=60)
print(a3,n=46)
...
#生育次数与生育年份(地区分层)
a4<-screening3%>%filter(生育年份<2011,生育年份>=1965)%>%group_by(地区,生育年份)%>%summarise(n=n(),mean=mean(生育次数,na.rm=TRUE))
print(a4,n=92)
a4%>%filter(n<100)
screening3%>%filter(生育年份<2003,生育年份>=1967)%>%group_by(生育年份,地区)%>%summarise(mean=mean(生育次数,na.rm=TRUE))%>%
...
#不拟合
birth_region<-screening3%>%filter(生育年份<2003,生育年份>=1967)%>%group_by(生育年份,地区)%>%summarise(mean=mean(生育次数,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=地区,linetype=地区))+geom_line()+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='生育次数(均值)',title='地区分层\n(n=73047)')



#生育次数与生育年份(教育分层)
a5<-screening3%>%filter(!is.na(教育))%>%group_by(生育年份,教育)%>%summarise(n=n())
print(a5,n=175)
a6<-a5%>%filter(n<50)
print(a6,n=59)
a7<-a6%>%group_by(生育年份)%>%summarise(n=n())
print(a7,n=32)
with(subset(screening3,生育年份<2011 &生育年份>=1965 & !is.na(教育)),table(is.na(生育年份),is.na(生育次数)))
with(subset(screening3,生育年份<1999 & 生育年份>=1969 & !is.na(教育)),table(is.na(生育年份),is.na(生育次数)))
screening3%>%filter(生育年份<1999,生育年份>=1969,!is.na(教育))%>%group_by(生育年份,教育)%>%summarise(mean=mean(生育次数,na.rm=TRUE))%>%
...
#不拟合
birth_educat<-screening3%>%filter(生育年份<1999,生育年份>=1969,!is.na(教育))%>%group_by(生育年份,教育)%>%summarise(mean=mean(生育次数,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=教育,linetype=教育))+geom_line()+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='生育次数(均值)',title='教育分层\n(n=68797)')
##合并教育和地区
...
#生育次数与生育年份(教育分层,地区分面)
screening3%>%filter(地区=='市区')%>%group_by(教育,出生年份)%>%summarise(n=n())
...
#初次生育年龄与生育年份(地区分层)
a8<-screening3%>%group_by(生育年份,地区)%>%summarise(n=n())%>%filter(n<50)%>%group_by(生育年份)%>%summarise(n=n())
print(a8,n=28)
with(subset(screening3,生育年份<2003 & 生育年份>1966),table(is.na(生育年份),is.na(初次生育年龄)))
screening3%>%filter(生育年份<2003,生育年份>1966)%>%group_by(生育年份,地区)%>%summarise(mean=mean(初次生育年龄,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=地区,linetype=地区))+geom_smooth(method='loess',se=FALSE)+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='初次生育年龄(均值)',title='地区分层\n(n=76158)')
#不拟合
age_region<-screening3%>%filter(生育年份<2003,生育年份>1966)%>%group_by(生育年份,地区)%>%summarise(mean=mean(初次生育年龄,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=地区,linetype=地区))+geom_line()+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='初次生育年龄(均值)',title='地区分层\n(n=73209)')
...
#初次生育年龄与生育年份(教育分层)
a9<-screening3%>%filter(!is.na(教育))%>%group_by(生育年份,教育)%>%summarise(n=n())%>%filter(n<50)%>%group_by(生育年份)%>%summarise(n=n())
print(a9,n=29)
with(subset(screening3,生育年份<1999 & !is.na(教育) & 生育年份>1968),table(is.na(生育年份),is.na(初次生育年龄)))

screening3%>%filter(生育年份<1999,!is.na(教育),生育年份>1968)%>%group_by(生育年份,教育)%>%summarise(mean=mean(初次生育年龄,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=教育,linetype=教育))+geom_smooth(method='loess',se=FALSE)+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='初次生育年龄(均值)')
#不拟合
age_educati<-screening3%>%filter(生育年份<1999,!is.na(教育),生育年份>1968)%>%group_by(生育年份,教育)%>%summarise(mean=mean(初次生育年龄,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=教育,linetype=教育))+geom_line()+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='初次生育年龄(均值)',title='教育分层\n(n=68946)')
#合并教育与地区
...
#哺乳时间
a10<-screening3%>%filter(!is.na(哺乳月份))%>%group_by(生育年份)%>%summarise(n=n())
print(a10,n=58)
...
#哺乳月份与生育年份(地区分层)
a11<-screening3%>%filter(!is.na(哺乳月份))%>%group_by(生育年份,地区)%>%summarise(n=n())%>%filter(n<50)%>%group_by(生育年份)%>%summarise(n=n())
print(a11,n=29)
with(subset(screening3,生育年份<2003 & !is.na(哺乳月份) & 生育年份>1967),table(is.na(生育年份),is.na(地区)))

screening3%>%filter(生育年份<2003  & 生育年份>1967)%>%group_by(生育年份,地区)%>%summarise(mean=mean(哺乳月份,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=地区,linetype=地区))+geom_smooth(method='loess',se=FALSE)+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='哺乳月份(均值)',title='地区分层\n(n=65187)')
#不拟合
breed_region<-screening3%>%filter(生育年份<2003  & 生育年份>1967)%>%group_by(生育年份,地区)%>%summarise(mean=mean(哺乳月份,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=地区,linetype=地区))+geom_line()+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='哺乳月份(均值)',title='地区分层\n(n=62601)')

#哺乳月份与生育年份(教育分层)

a12<-screening3%>%filter(!is.na(哺乳月份),!is.na(教育))%>%group_by(生育年份,教育)%>%summarise(n=n())%>%filter(n<50)%>%group_by(生育年份)%>%summarise(n=n())
print(a12,n=29)
with(subset(screening3,生育年份<1999 & !is.na(哺乳月份) & 生育年份>1968),table(is.na(生育年份),is.na(教育)))
#不拟合
breed_educati<-screening3%>%filter(生育年份<1999 & 生育年份>1968,!is.na(教育))%>%group_by(生育年份,教育)%>%summarise(mean=mean(哺乳月份,na.rm=TRUE))%>%
  ggplot(aes(x=生育年份,y=mean,color=教育,linetype=教育))+geom_line()+
  scale_x_continuous(breaks = seq(1960,2019,5))+mytheme+labs(y='哺乳月份(均值)',title='教育分层\n(n=59040)')
#合并教育与地区
...
#绝经年龄
#绝经年龄与出生年份
a13<-screening3%>%filter(!is.na(绝经年龄),绝经年龄>=50)%>%group_by(出生年份)%>%summarise(n=n())
print(a13,n=29)
num<-screening3%>%filter(!is.na(绝经年龄),绝经年龄>=50)%>%group_by(出生年份)%>%summarise(n=n(),mean=mean(绝经年龄,na.rm=TRUE))
print(num,n=35)
with(subset(screening3,绝经年龄=50 & 出生年份<=1966),table(is.na(出生年份),is.na(绝经年龄)))
screening3%>%filter(绝经年龄>=50,出生年份<=1966)%>%group_by(出生年份)%>%summarise(mean=mean(绝经年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean))+geom_smooth(method='loess',se=FALSE)+geom_point()+
  scale_x_continuous(breaks = seq(1945,1979,5))+mytheme+labs(y='绝经年龄(均值)',title='绝经年龄\n(n=56244)')
#不拟合
screening3%>%filter(绝经年龄>=50,出生年份<=1966,绝经=='是')%>%group_by(出生年份)%>%summarise(mean=mean(绝经年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean))+geom_line(colour='blue')+
  scale_x_continuous(breaks = seq(1945,1979,5))+mytheme+labs(y='绝经年龄(均值)',title='绝经年龄\n(n=58822)')
...
screening3%>%group_by(地区)%>%summarise(mean=mean(绝经年龄,na.rm=T),sd=sd(绝经年龄,na.rm=T))
with(subset(screening3,绝经年龄>50 & 出生年份<=1960),table(is.na(出生年份),is.na(绝经年龄)))
screening3%>%filter(绝经年龄>=50,出生年份<=1960,绝经=='是')%>%group_by(出生年份,地区)%>%summarise(mean=mean(绝经年龄,na.rm=TRUE))%>%
...
#绝经年龄与出生年份(教育分层)
screening3%>%filter(绝经年龄>50,出生年份<=1970,!is.na(教育))%>%group_by(出生年份,教育)%>%summarise(mean=mean(绝经年龄,na.rm=TRUE))%>%
  ggplot(aes(x=出生年份,y=mean,color=教育,linetype=教育))+geom_smooth(method='loess',se=FALSE)+
...
#绝经年龄与BMI
screening3%>%filter(绝经年龄>=50)%>%group_by(绝经年龄)%>%summarise(n=n(),BMI=mean(BMI,na.rm=T))
screening3%>%filter(绝经年龄>=50)%>%group_by(绝经年龄)%>%summarise(BMI=mean(BMI,na.rm=T))%>%
